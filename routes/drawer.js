...
router.post("/", protect, async (req, res) => {
  ...
  try {
    ...
    // If the drawer doesn't exist, create a new one with the new item
    if (!drawer) {
      drawer = new Drawer({ user: req.user._id, items: [filteredBody] });
    } else {
      // Check if an item with the same name already exists in the drawer
      const duplicateItem = drawer.items.find(
        (item) => item.name.toLowerCase() === filteredBody.name.toLowerCase()
      );

      if (duplicateItem) {
        return res.status(400).json({
          message: "Item with this name already exists in the drawer",
        });
      }

      // If no duplicate, add the new item to the drawer
